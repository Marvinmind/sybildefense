...
for sys in ('integro', ):
	...
	for enu, i in enumerate((0.1, 0.2, 0.4)):
		...
		SRall = pickle.load(open('../pickles/victimProb/victimProb{}SRTar.p'.format(i), 'rb'))
		Pall = pickle.load(open('../pickles/victimProb/victimProb{}PTar.p'.format(i), 'rb'))

		pRes = Pall[0]
		pResAUC = getMergedAuc(pRes)
		print(pResAUC)
		print('auc peripheral')
